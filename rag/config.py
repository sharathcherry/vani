...
# ── Qdrant Vector Store (DEPRECATED — kept for fallback only) ────────────────
# Phase 2 migration: primary retriever now uses Azure AI Search (see below).
...
# ── Azure AI Search (primary vector + BM25 + RRF + semantic reranker) ────────
# Required for Phase 2 hybrid retrieval. NEVER commit AZURE_SEARCH_API_KEY.
AZURE_SEARCH_ENDPOINT = os.getenv("AZURE_SEARCH_ENDPOINT", "")
AZURE_SEARCH_API_KEY = os.getenv("AZURE_SEARCH_API_KEY", "")
AZURE_SEARCH_INDEX = os.getenv("AZURE_SEARCH_INDEX", "gov-schemes")
AZURE_SEARCH_SEMANTIC_CONFIG = os.getenv("AZURE_SEARCH_SEMANTIC_CONFIG", "default")
# Toggle server-side semantic reranker. Phase 2 evaluation flag.
AZURE_SEARCH_USE_SEMANTIC = os.getenv("AZURE_SEARCH_USE_SEMANTIC", "false").lower() == "true"
# k_nearest_neighbors for vector leg of hybrid query.
AZURE_SEARCH_VECTOR_K = int(os.getenv("AZURE_SEARCH_VECTOR_K", "50"))
# Top results to pull from Azure before passing to BGE reranker.
AZURE_SEARCH_TOP = int(os.getenv("AZURE_SEARCH_TOP", "20"))
...
# ── LLM ───────────────────────────────────────────────────────────────────────
# Supported providers: groq | openai | sagemaker | azure
# Groq is the fastest and cheapest option — free tier handles production load.
# Azure OpenAI added in Phase 1 for enterprise-grade hosted inference.
...
# ── Azure OpenAI (Phase 1 — hosted chat completion provider) ─────────────────
# NEVER commit AZURE_OPENAI_API_KEY. Required only when LLM_PROVIDER=azure.
