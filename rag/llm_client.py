"""Pluggable LLM client — Groq (default), OpenAI, Azure OpenAI, or AWS SageMaker.
...
Azure OpenAI (Phase 1):
  Provision a chat-completion deployment in Azure AI Foundry, then set:
  AZURE_OPENAI_ENDPOINT=https://<resource>.openai.azure.com
  AZURE_OPENAI_API_KEY=<key>                      # never commit
  AZURE_OPENAI_API_VERSION=2024-08-01-preview     # or current GA
  AZURE_OPENAI_CHAT_DEPLOYMENT=<deployment-name>  # e.g. gpt-4o-mini-prod
  LLM_PROVIDER=azure
"""
...
class AzureOpenAIClient:
    ...
    Mirrors the BedrockClient / GroqClient interface used elsewhere in the
    codebase: a single ``generate(messages, max_tokens, temperature, stop)``
    method that returns the assistant's text response.

    The deployment name (not the model id) is required by Azure OpenAI — it
    identifies the specific provisioned deployment in your Azure resource.
    """
    ...
    def __init__(self) -> None:
        ...
        self._deployment = AZURE_OPENAI_CHAT_DEPLOYMENT
    ...
    def generate(
        ...
        """Same signature as BedrockClient/GroqClient generate()."""
        kwargs: dict = {
