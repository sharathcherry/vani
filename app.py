...
def get_rag_answer(english_query: str) -> tuple[str, list[dict], dict]:
    """Invoke the local RAG Pipeline directly.

    Returns:
        answer   -- generated English answer text
        contexts -- list of source chunks
        filters  -- metadata filters applied by the pipeline
    """
    from rag.pipeline import RAGPipeline
    
    # We use a singleton instance for Streamlit
    if "rag_pipeline" not in st.session_state:
        st.session_state["rag_pipeline"] = RAGPipeline()
        
    pipeline = st.session_state["rag_pipeline"]
    try:
        result = pipeline.answer(english_query)
        contexts = result.retrieved_chunks
        return result.answer, contexts, result.metadata_filter
    except Exception as e:
        st.error(f"RAG Pipeline call failed: {e}")
